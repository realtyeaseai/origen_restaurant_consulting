...
export default function Header() {
  ...
  const openCalendly = () => {
    ...
    // Fallback when popup blockers prevent opening a new window.
    if (!popup) window.open(url, "_blank");
  };

  const navLinkClass = (path) =>
    `text-base lg:text-lg flex items-center h-10 whitespace-nowrap text-amber-500 hover:text-amber-600 transition-colors ${
      pathname === path
        ? "underline underline-offset-4 decoration-2 decoration-amber-500"
        : ""
    }`;
